...
/*
 * This is the class from HW4, it uses new evaluation function rather than the old score().
 */
public class OthelloEvalPlayer_ShangqiWu extends OthelloPlayer {
	
	int depth_bound;
	int player_number;
	
	public OthelloEvalPlayer_ShangqiWu(int _depth_bound, int _player_number) {
		if (_depth_bound > 0) depth_bound = _depth_bound;
	...
	private Integer MinValue(OthelloState _min_state, int _min_depth) {
		...
		if (_min_depth < depth_bound && !_min_state.gameOver()) {
			...
			for (int i = 0; i < minMove.size(); i++) {
				...
				if (minValue > fromMaxValue) minValue = fromMaxValue;
			}
		} else {
			// If terminal-test(state), return utility value.
			minValue = _min_state.eval();
			if (player_number == 1) minValue = -minValue;
		}
		return minValue;
	}
	...
	private Integer MaxValue(OthelloState _max_state, int _max_depth) {
		...
		if (_max_depth < depth_bound && !_max_state.gameOver()) {
			...
			for (int i = 0; i < maxMove.size(); i++) {
				...
				if (maxValue < fromMinValue) maxValue = fromMinValue;
			}
		} else {
			maxValue = _max_state.eval();
			if (player_number == 1) maxValue = -maxValue;
		}
		return maxValue;
	}
}
